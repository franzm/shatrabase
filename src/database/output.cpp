...
void Output::writeMove(MoveToWrite moveToWrite)
{
    ...
    if (moveToWrite == NextMove) moveId = m_game->nextMove();
    else moveId = m_game->currentMove();
    ...
    if (m_game->nags(moveId).count() > 0)
    {
        if (m_options.getOptionAsBool("SymbolicNag"))
			...
			nagString += m_game->nags(moveId).toString(NagSet::SGN);
	}
 // Read comments
    ...
    {
        m_output += m_startTagMap[MarkupColumnStyleRow] +
                m_startTagMap[MarkupColumnStyleMove];
	}
	...
	while (!m_game->atLineEnd()) {
		...
		if (m_game->variationCount()) {
			...
			for (int i = 0; i < variations.size(); ++i) {
				...
				// *** End the variation
				//			m_output.replace ( QRegExp ("\\s+$"), "" );
				// We don't want any spaces before the )
				if (m_currentVariationLevel <= m_options.getOptionAsInt("VariationIndentLevel")) {
