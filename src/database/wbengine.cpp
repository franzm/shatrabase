...
void WBEngine::parseAnalysis(const QString& message)
{
	...
	MoveList moves;
	int section = 4;
	while ((lannMove = trimmed.section(' ', section, section)) != "") {
		if (lannMove.startsWith("(")) {
			break;
		}
		// SBE -- What the heck is "<HT>" and why does crafty send it?
		if (!lannMove.endsWith(".") && lannMove != "<HT>") {
			Move move = board.parseMove(lannMove);
			if (!move.isLegal()) {
//				qWarning("! move: |%s|", lannMove.toLatin1().constData());
